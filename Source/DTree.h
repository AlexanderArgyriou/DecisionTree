...
class DTree
{
	...
	public:
		...
		inline float AvgQuestionsToPredict() const noexcept;        // AvgQuestionsToPredict
		inline void Test(const Vec_vec_str &Descr) const noexcept;  // Test
};	// DTree
...
inline void DTree::BuildTree(Node *Tree) noexcept
{
	...
	if (Qst.first != "" && Qst.second != -1)
	{
		...
		float ImYes = CaclulateImpurity(DataSetYes);
		...
		float ImNo = CaclulateImpurity(DataSetNo);
...
DTree::FindTheRightQuestion(const Vec_vec_str &Set) const noexcept
{
	...
	Pair_PairStrFloat_Int Ans{ { "", 0.0f }, -1 };
...
inline float 
DTree::CaclulateImpurity(const Vec_vec_str &DataSet) const noexcept
{
	Map_Str_Sz_t Occ;
	float        Impurity = 1.0f;
	...
	const Vec_vec_str &MainSet) const noexcept
{
	...
	return CaclulateImpurity(MainSet) - P * CaclulateImpurity(DataSetY) - ( 1 - P ) * CaclulateImpurity(DataSetN); // same simpler result
	/*return ( ((float)DataSetN.size() + (float)DataSetY.size()) / TrainSet->size() ) * 
		( CaclulateImpurity(MainSet) - P * CaclulateImpurity(DataSetY) - (1 - P) * CaclulateImpurity(DataSetN) );*/
...
inline void DTree::Test(Node *Tree,
	const Vec_str &Descr) const noexcept
{
	if (Tree)
	{
		if (Tree->Leaf)
		{
			Vec_str CutLabel(Tree->Data[0].begin() + 1, Tree->Data[0].end());
			if (CutLabel == Descr)
			{
		...
		Test(Tree->Children[Yes], Descr);
		Test(Tree->Children[No], Descr);
	}	// if
}	// Test
...
inline float DTree::AvgQuestionsToPredict() const noexcept
{
	Vec_sz_t    LeafsPerLevel(TreeHeight(Root), 0);
	std::size_t NumOfQuestions = 0;

	LeafNodesPerLvl(Root, LeafsPerLevel, 0);

	for (std::size_t i = 1; i < LeafsPerLevel.size(); ++i)
		NumOfQuestions += i * LeafsPerLevel[i];

	return (float)NumOfQuestions / (float)TrainSet->size();
}	// AvgQuestionsToPredict

inline void DTree::Test(const Vec_vec_str &TD) const noexcept
{
	for(const auto &i : TD)
		Test(Root, i);
}	// Test